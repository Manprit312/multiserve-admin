...
import { Sparkles, Loader2, Upload, Plus, Trash2, X } from "lucide-react";
...
const [form, setForm] = useState<BannerForm>({
  ...
  useEffect(() => {
    async function fetchData() {
      const res = await fetch(`${API_BASE}/api/home-banners/${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await fetch(`${API_BASE}/api/home-banners/${id}`, { method: "PUT", body: fd });
  ...
  return (
    <section className="relative bg-gradient-to-b from-blue-50 to-sky-100 py-16">
      ...
      <div className="max-w-3xl mx-auto bg-white p-8 rounded-2xl shadow border border-blue-100">
        <h1 className="text-2xl font-bold mb-6 text-slate-800">Edit Home Banner</h1>
...
/>
          ...
          <button
            disabled={saving}
            type="submit"
            className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-sky-500 text-white font-semibold px-5 py-2 rounded-full shadow hover:shadow-lg disabled:opacity-70"
          >
            {saving ? <Loader2 className="animate-spin" /> : <Upload size={18} />}{" "}
